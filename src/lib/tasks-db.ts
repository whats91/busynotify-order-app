/*
 * File Context:
 * Purpose: Provides shared infrastructure for Tasks Db.
 * Primary Functionality: Supports shared runtime infrastructure consumed by routes, services, or server helpers.
 * Interlinked With: No direct internal imports; primarily used by framework or toolchain entry points.
 * Role: shared infrastructure.
 */
import { mkdirSync } from 'node:fs';
import path from 'node:path';
import { PrismaClient } from '@prisma/client';

const FALLBACK_DB_PATH = path.join(process.cwd(), 'data', 'busy-notify-tasks.sqlite');

const globalForTaskPrisma = globalThis as unknown as {
  taskPrisma: PrismaClient | undefined;
};

function normalizeTaskDatabaseUrl(databaseUrl?: string): string {
  const rawUrl = databaseUrl?.trim();

  if (rawUrl?.startsWith('file:')) {
    const filePath = rawUrl.slice(5);
    const resolvedPath = filePath
      ? path.isAbsolute(filePath)
        ? filePath
        : path.resolve(process.cwd(), filePath)
      : FALLBACK_DB_PATH;

    mkdirSync(path.dirname(resolvedPath), { recursive: true });
    return `file:${resolvedPath}`;
  }

  if (
    rawUrl &&
    (rawUrl.endsWith('.db') || rawUrl.endsWith('.sqlite') || rawUrl.endsWith('.sqlite3'))
  ) {
    const resolvedPath = path.isAbsolute(rawUrl)
      ? rawUrl
      : path.resolve(process.cwd(), rawUrl);

    mkdirSync(path.dirname(resolvedPath), { recursive: true });
    return `file:${resolvedPath}`;
  }

  mkdirSync(path.dirname(FALLBACK_DB_PATH), { recursive: true });
  return `file:${FALLBACK_DB_PATH}`;
}

const TASK_DATABASE_URL = normalizeTaskDatabaseUrl(process.env.TASKS_DATABASE_URL);

export const taskDb =
  globalForTaskPrisma.taskPrisma ??
  new PrismaClient({
    datasources: {
      db: {
        url: TASK_DATABASE_URL,
      },
    },
    log: ['query'],
  });

if (process.env.NODE_ENV !== 'production') {
  globalForTaskPrisma.taskPrisma = taskDb;
}
